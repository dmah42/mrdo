...
llvm::Value* Variable::Codegen() const {
  ...
  if (!val) {
    Error("Unknown variable name: ", name_, " at ", lexer::line, "|", lexer::col);
    return nullptr;
  }
...
llvm::Value* BinaryOp::Codegen() const {
  ...
  if (op_ == "=") {
    ...
    if (!lhs_expression) {
      Error("LHS of assignment must be a variable at ", lexer::line, "|", lexer::col);
      return nullptr;
    }
  ...
  Error("unknown binary operator at ", lexer::line, "|", lexer::col);
  return nullptr;
}
...
llvm::Value* UnaryOp::Codegen() const {
  ...
  if (op_ == "not")
    return builder.CreateUIToFP(
        ...
        llvm::Type::getDoubleTy(llvm::getGlobalContext()), "booltmp");
  Error("unknown unary operator at ", lexer::line, "|", lexer::col);
  return nullptr;
}
